...
def index(request):
    ...
    delta_x = max(x) - min(x)
    x_start = min(x) - 0.05 * delta_x
    x_end = max(x) + 0.05 * delta_x

    p1 = figure(height=280, width=900, tools='wheel_zoom, box_zoom, reset', x_range=(x_start, x_end),
                x_axis_type='datetime', y_axis_label='mm Hg')
    ...
    p2 = figure(height=180, width=900, tools='wheel_zoom, box_zoom, reset', x_axis_type='datetime',
                y_axis_label='Pulse')
